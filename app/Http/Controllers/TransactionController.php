...
class TransactionController extends Controller
{
    ...
    public function sendMoneyToClient(Request $request)
    {
        ...
        $request->validate([
            'montant' => 'required|numeric|min:500',
            'telephone' => 'required', // Assurez-vous que la table est correcte
        ]);
        
       
        $userFrom = Auth::user();
        if(empty($userTo) || $userFrom->archived == 0) {
            return redirect()->back()->with('error','Le Client  n\'existe pas ou est bloqué.');
        }
        $clientFrom = Client::where('id_user', $userFrom->id)->first();
    
        $userTo = User::where('telephone', $request->telephone)->first();
        $clientTo = Client::where('id_user', $userTo->id)->first();
        ...
        Transaction::create([
            'id_compte_source' => $userFrom->id,
            'id_compte_destinataire' => $userTo->id,
